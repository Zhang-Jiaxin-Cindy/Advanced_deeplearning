...
class Block(nn.Module): # Feature-wise Linear Modulation
    def __init__(self, dim, dim_out, groups=8):
        super().__init__()
        self.proj = WeightStandardizedConv2d(dim, dim_out, 3, padding=1)
        self.norm = nn.GroupNorm(groups, dim_out)
        self.act = nn.SiLU() # 它比 ReLU 更平滑
...
class Unet(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv2d(input_channels, init_dim, 1, padding=0)  # type: ignore # changed to 1 and 0 from 7,3

        dims = [init_dim, *map(lambda m: dim * m, dim_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))

        block_class = partial(ResnetBlock, groups=resnet_block_groups) # skip connection
        ...
        if self.class_free_guidance:
            self.class_emb = nn.Embedding(num_classes + 1, time_dim)  # type: ignore
            with torch.no_grad():
                self.class_emb.weight[-1].fill_(0.0)
        classes_dim = time_dim if self.class_free_guidance else None
        # layers
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        ...
                        block_class(dim_in, dim_in, time_emb_dim=time_dim, classes_emb_dim=classes_dim),
                        block_class(dim_in, dim_in, time_emb_dim=time_dim, classes_emb_dim=classes_dim),
        ...
        self.mid_block1 = block_class(mid_dim, mid_dim, time_emb_dim=time_dim, classes_emb_dim=classes_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = block_class(mid_dim, mid_dim, time_emb_dim=time_dim, classes_emb_dim=classes_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        block_class(dim_out + dim_in, dim_out, time_emb_dim=time_dim, classes_emb_dim=classes_dim),
                        block_class(dim_out + dim_in, dim_out, time_emb_dim=time_dim, classes_emb_dim=classes_dim),
        ...
        self.final_res_block = block_class(dim * 2, dim, time_emb_dim=time_dim, classes_emb_dim=classes_dim)
        self.final_conv = nn.Conv2d(dim, self.out_dim, 1) # type: ignore
    ...
    def forward(self, x, time, class_labels=None):
        ...
        if self.class_emb is not None:
            ...
            if self.training:
                ...
                if self.p_uncond is not None:
                    prob_mask = torch.rand(x.shape[0], device=x.device) < self.p_uncond
                    if prob_mask.any():
                        class_labels = class_labels.clone() 
                        class_labels[prob_mask] = self.num_classes
        ...
        # Middle block
        x = self.mid_block1(x, time_emb=t, class_emb=class_emb)
        x = self.mid_attn(x)
        x = self.mid_block2(x, time_emb=t, class_emb=class_emb)
        ...
        x = self.final_res_block(x, time_emb=t, class_emb=class_emb)
